...
exports.createGuide=function({app,BrowserWindow,ipcMain,screen,desktopCapturer,globalShortcut,systemPreferences,shell,permissions,authorize,apiUrl,launchTarget=async()=>{throw new Error('Opening apps and websites is unavailable.');},fetcher=fetch,inputDriver=createInput(app),wait=delay}){
 ...
 const idle=()=>({started:false,goal:'',busy:false,phase:'idle'});let state=idle();
 ...
 function progress(token,phase){if(current(token)){state={...state,phase};publish();}}
 function stop(){
 ...
 async function next(goal,verifyOnly=false){
  ...
  const checking=verifyOnly||state.executed===true;
  if(state.step){token.history.push(state.step);token.history=token.history.slice(-4);}
  voiceEpoch++;token.pending=null;state={...state,started:true,goal:goal.trim(),busy:true,phase:checking?'checking':'preparing',step:null,executed:false,canExecute:false,error:'',retryable:false};publish();marker.hide();
  try{
   ...
   if(['click','type'].includes(step.action)&&status?.trusted){
    progress(token,'locating');
    const located=await input.run({action:'locate_control',label:step.target,kind:step.action==='type'?'text':'control',pid:status.pid},token.controller.signal);
    if(!current(token))return state;
   ...
   state={...state,busy:false,phase:'ready',step,canExecute:['open_website','open_app'].includes(step.action)||(['click','double_click','type','scroll'].includes(step.action)&&Boolean(status?.trusted&&status.pid&&status.pid!==process.pid)),needsPermission:process.platform==='darwin'&&!status?.trusted,error:helperError};
  ...
  }catch(e){if(current(token)){token.pending=null;state={...state,busy:false,phase:'error',step:null,error:e.message};show();publish();}}
  return state;
 }
 async function execute(){
  ...
  voiceEpoch++;token.pending=null;state={...state,busy:true,phase:'checking_target'};publish();marker.hide();
  try{
   ...
   if(['open_website','open_app'].includes(pending.step.action)){
    progress(token,'acting');
    await launchTarget(pending.step);if(!current(token))return state;
    state={...state,busy:false,phase:'ready',executed:true,canExecute:false,error:''};show();return publish();
   }
   ...
   if(!current(token))return state;
   state={...state,busy:false,phase:'ready',executed:true,canExecute:false,error:''};show();publish();
  }catch(e){if(current(token)){state={...state,busy:false,phase:'error',canExecute:false,error:e.message,retryable:e.code==='GUIDE_SCREEN_CHANGED'};show();publish();}}
  return state;
 }
 ...
 ipcMain.handle('guide:voice',async(event,text,epoch)=>{
  ...
  if(!command){
   ...
   if(url||name){
    ...
    const token=run;voiceEpoch++;token.pending=null;marker.hide();state={...state,busy:true,phase:'acting'};publish();
    try{const result=await launchTarget(url?{action:'open_website',url}:{action:'open_app',name});
     if(!current(token))return {stopped:true};
     state={...state,busy:false,phase:'ready',started:true,goal:state.goal||text,canExecute:false,executed:false,step:{action:'manual',instruction:`Opened ${result.name}. Say “I did it” when you’re ready to continue.`},error:''};
    }catch(e){if(current(token))state={...state,busy:false,phase:'error',canExecute:false,error:e.message};}
    return publish();
   }
  }
  if(!command){
 ...
 ipcMain.handle('guide:enable',async event=>{
  ...
  state={...state,canExecute:false,error:'Allow Acumen / Electron in Accessibility, return to your task, then get a fresh step.'};if(run)run.pending=null;return publish();
 });
