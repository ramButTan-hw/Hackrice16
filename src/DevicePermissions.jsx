...
export default function DevicePermissions(){
  ...
  return <details className="details"><summary>Device permissions</summary><div className="detail-body">
    <p>Camera supports biometrics and attention. Microphone supports spoken input. Screen access is used when you request screen help. Accessibility lets Guide click and type when you choose “Do it for me.”</p>
    {['camera','microphone','screen','accessibility'].map(kind=><div className="device-permission" key={kind}><span>{kind==='screen'?'Screen Recording':kind==='camera'?'Camera':kind==='accessibility'?'Accessibility · Input control':'Microphone'} · {state[kind]}</span><button type="button" disabled={busy||state[kind]==='granted'} onClick={()=>act(kind,state[kind]!=='not-determined')}>{state[kind]==='granted'?'Allowed':state[kind]==='not-determined'?'Allow':'Open Settings'}</button></div>)}
    <p>Enable Electron during development, or Acumen for an installed app. After changing access in System Settings, fully quit and reopen the app. Resume the camera or restart the demo to retry.</p>
