...
export default function Planner({ api }) {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([api('/analytics/patterns?timeZone=' + encodeURIComponent(timeZone)), api('/plans')]).then(([p, rows]) => { if (!cancelled) { setPatterns(p); setPlans(rows); } }).catch(e => { if (!cancelled) setError(e.message); });
  ...
  async function publish(plan) {
    ...
    try {
      ...
      setSlots(null); setNotice('Added to Google Calendar · Jarvis work sessions.');
    } catch (e) { setError(e.message + ' Your Jarvis plan is saved. Retrying uses the same Google event ID.'); }
  ...
  async function save(slot) {
    setBusy(true); setError('');
    try { const plan = await api('/plans', { title, start: slot.start, end: slot.end, timeZone }); setPlans(previous => [...previous, plan].sort((a, b) => a.start - b.start)); setSlots(null); setNotice('Saved to your Jarvis schedule.'); }
    catch (e) { setError(e.message); } finally { setBusy(false); }
  }
  ...
  return <section className="planner calendar-planner" aria-label="Schedule planner">
    <header className="planner-heading"><div><h1>Plan your day.</h1><p>Pick a task. Make a little room to focus.</p></div><button className="google-status-button" disabled={busy} onClick={connect}><span className={google?.connected?'connection-dot connected':'connection-dot'}/>{google?.connected?'Google connected':'Connect Google'}</button></header>
    ...
    <div className="planner-layout"><div className="planner-primary">
      <section className="month-card" aria-label="Monthly calendar">
        ...
        <footer className="calendar-legend"><span><i/> Jarvis work blocks</span><span>{timeZone.replaceAll('_',' ')}</span></footer>
      </section>
      <section className="day-agenda" aria-label="Selected day plans"><header><div><span className="eyebrow">YOUR DAY</span><h2>{selectedLabel}</h2></div><span className="plan-count">{dayPlans.length} {dayPlans.length===1?'block':'blocks'}</span></header>
      {!dayPlans.length?<div className="agenda-empty"><span aria-hidden="true">◷</span><div><h3>A little breathing room.</h3><p>No work blocks yet. Pick a task and find a time that fits.</p></div></div>:dayPlans.map(plan=><article className="agenda-item" key={plan.id}><div className="agenda-time"><strong>{time(plan.start)}</strong><span>{time(plan.end)}</span></div><div className="agenda-info"><h3>{plan.title}</h3><p>{Math.round((plan.end-plan.start)/60000)} min · {plan.google?'Added to Google':'Jarvis plan'}</p><div className="agenda-actions">{plan.google?<button onClick={()=>void openGoogle(plan.google.url).catch(e=>setError(e.message))}>Open in Google ↗</button>:<button disabled={busy||!google?.connected||plan.end<=Date.now()} onClick={()=>publish(plan)}>Add to Google</button>}<button disabled={busy} onClick={()=>remove(plan.id)}>{plan.google?'Remove from Jarvis only':'Remove'}</button></div></div></article>)}
      </section>
      ...
      <section className="rhythm-summary"><span className="eyebrow">LEARNING YOUR RHYTHM</span><h3>{knownHours?'Your patterns are taking shape.':'Every session adds perspective.'}</h3><p>{patterns?`${patterns.sessionCount} completed sessions · ${knownHours} hours with enough history`:'Loading your session history…'}</p><details><summary>View signal patterns</summary><div className="hour-grid">{patterns?.hours.map(h=><div key={h.hour} className={h.score===null?'unknown':h.score>=40?'elevated':'steady'} title={`${h.hour}:00 · ${h.dayCount} days · ${Math.round(h.observedSeconds/60)} minutes`}><span>{String(h.hour).padStart(2,'0')}</span><strong>{h.score===null?'—':`${h.score}%`}</strong></div>)}</div><p>Each hour needs 10 recorded minutes across 3 sessions on 3 days. Percentages show elevated-signal time, not stress probability. Demo data is excluded.</p></details></section>
      <details className="calendar-settings"><summary>Calendar connection <span>{google?.connected?'Connected':'Not connected'}</span></summary><p>{google?.email??'Connect Google to check availability and publish blocks.'}</p>{google?.connected&&<label className="calendar-toggle"><input type="checkbox" checked={useGoogle} disabled={busy} onChange={e=>{setUseGoogle(e.target.checked);setSlots(null);}}/> Check Google busy times</label>}<p>Availability checks cover your primary and Jarvis calendars. The calendar above shows saved Jarvis blocks only. Google edits are managed in Google.</p><button disabled={busy} onClick={connect}>{google?.connected?'Reconnect Google':'Connect Google'}</button></details>
