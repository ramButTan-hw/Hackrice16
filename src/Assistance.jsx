...
import {responseStyle,wakeGreeting} from '../shared/response-style.js';
...
export default function Assistance({session,responseContext,onSessionEnded=()=>{}}){
  ...
  const [data,setData]=useState({}),[open,setOpen]=useState(false),[text,setText]=useState(''),[evidence,setEvidence]=useState(null),[error,setError]=useState(''),[now,setNow]=useState(Date.now());
  const seen=useRef(null),callbacks=useRef({});callbacks.current={begin,answer};
  const [ready,setReady]=useState(false),[listening,setListening]=useState(false),[listenId,setListenId]=useState(0),[wake,setWake]=useState(()=>!rehearsal&&localStorage.getItem('companion.handsfree')!=='false');
  ...
  const wakeStatus=useWake(bridge,available&&ready&&wake&&!listening&&!rehearsal,onWake);
  function begin(checkin='',proof=null){setText(checkin);setEvidence(proof);setGreeting(rehearsal?'':wakeGreeting(responseStyle({...responseContext,feeling:data.feeling??responseContext?.feeling})));setOpen(true);setListening(!rehearsal);if(!rehearsal)setListenId(n=>n+1);}
  ...
  useEffect(()=>{
    ...
    const timer=setInterval(async()=>{setNow(Date.now());if(!activeRef.current||pending)return;pending=true;try{const next=await api(id);if(alive){setData(next);if(next.checkin&&seen.current!==next.checkin.id){seen.current=next.checkin.id;callbacks.current.begin(next.checkin.text,next.checkin.evidence);void bridge?.notify?.(next.checkin).catch(e=>setError(e.message));}}}catch(e){if(alive)setError(e.message);}finally{pending=false;}},2000);
    return()=>{alive=false;clearInterval(timer);off?.();void bridge?.panel?.(null);void bridge?.session(null);};
  },[id,available]);
  useEffect(()=>{void bridge?.panel?.(open?{sessionId:id,transcript:text,listenId,evidence,greeting,responseContext}:null).catch(e=>setError(e.message));},[open,id,text,listenId,evidence,greeting,responseContext]);
  if(!available)return null;
  return <section className="assistance-card">
    <div className="companion-summary"><div><span className="eyebrow">ACUMEN</span><p role="status">{!active?'Session ended · Here if you need me':listening?'Here to help':rehearsal?'Rehearsal · Type to try local commands':wakeStatus}</p></div><button className="companion-open" onClick={()=>begin()}>Ask Acumen <span aria-hidden="true">↗</span></button></div>
    ...
    <details className="companion-options"><summary>Voice & preferences</summary><label><input type="checkbox" disabled={rehearsal} checked={wake} onChange={e=>{setWake(e.target.checked);localStorage.setItem('companion.handsfree',String(e.target.checked));}}/> Listen for “hey Jarvis”</label><p>{rehearsal?'Offline rehearsal uses typed commands. Run Acumen normally to test the microphone and wake phrase.':'Say “hey Jarvis,” wait for the chime, then ask. Pause to send. You can also type in the companion.'}</p><p>Screen requests capture once. Audio and screenshots are not saved. Manage work memory in the companion menu.</p><div className="companion-meta"><span>Break score {data.score??0} / 5</span><span>Pulse · {data.pulse?.phase?.replaceAll('_',' ')??'calibrating'}</span></div></details>
    {error&&<p className="inline-alert" role="alert">{error}</p>}
    {open&&!bridge?.panel&&<div className="help-popup"><CompanionChat sessionId={id} initialText={text} initialEvidence={evidence} greeting={greeting} responseContext={responseContext} listenId={listenId} onAction={value=>void answer(value)}/><button onClick={()=>void answer('close')}>Close</button></div>}
  </section>;
}
