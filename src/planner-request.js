...
export async function openPlanner(surface = window) {
  ...
  if (surface.helpPanel || surface.companionWindow) throw new Error('Restart Jarvis to enable opening the planner from chat.');
  const popup = surface.open('/?planner-window=1', 'jarvis-planner');
  if (!popup) throw new Error('Your browser blocked the planner window. Allow pop-ups for Jarvis and try again.');
  popup.opener = null;
}
