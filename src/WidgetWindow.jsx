...
export default function WidgetWindow({kind}){
  ...
  useEffect(()=>{
    alive.current=true;document.documentElement.classList.toggle('desktop',Boolean(controls));document.title='Jarvis '+kind;
    let pending=false;
    async function refresh(){if(pending)return;pending=true;try{const next=await fetch('/api/widgets/'+kind).then(jsonResponse);if(alive.current){if(!initialized.current&&kind==='timer'){setMinutes(next.minutes);initialized.current=true;}apply(next);}}catch(e){if(alive.current)setError(e.message);}finally{pending=false;}}
  ...
  return <div className="stage widget-stage"><main className={'glass-shell widget-shell '+(kind==='timer'?'compact-timer':'')} aria-label={'Jarvis '+kind+' widget'}><header className="topbar"><div className="drag window-drag-area app-wordmark"><span aria-hidden="true">✦</span>Jarvis <small>{kind==='timer'?'Timer':'Checklist'}</small></div><div className="window-actions">{kind!=='timer'&&<ThemePicker settings={appearance} onChange={setAppearance}/>}{controls&&<><button className={'icon-button '+(pinned?'selected':'')} aria-label="Keep widget on top" aria-pressed={pinned} onClick={async()=>{try{await controls.pinned(!pinned);setPinned(!pinned);}catch(e){setError(e.message);}}}>⌖</button>{kind!=='timer'&&<button className="icon-button" aria-label="Minimize widget" onClick={()=>controls.minimize()}>−</button>}<button className="icon-button" aria-label="Close widget" onClick={()=>controls.close()}>×</button></>}</div></header>
    <div className="content widget-content">{!data?<p role="status">Loading your {kind}…</p>:kind==='timer'?<>
      ...
      {!data.items.length&&<div className="widget-empty"><span aria-hidden="true">☑</span><p>Add your first step, or ask Jarvis<br/>to make a checklist for a task.</p></div>}
