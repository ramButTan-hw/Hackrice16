...
export default function Attention({video,sessionId,running}){
  ...
  useEffect(()=>{
    ...
    async function frame(){
      ...
      if(paused.current){calibration=[];void post('paused');setStatus('Paused while talking with Acumen');timer=setTimeout(frame,2000);return;}
    ...
    worker.onmessage=({data})=>{
      ...
      if(paused.current){state='paused';setStatus('Paused while talking with Acumen');}
