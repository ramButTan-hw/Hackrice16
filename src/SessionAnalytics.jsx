...
export default function SessionAnalytics({ session, api, onPlan }) {
  ...
  return <section className="insight-card" aria-label="Session analytics">
    <span className="eyebrow">ACUMEN · SESSION COMPLETE</span>
    {mode === 'prompt' ? <><h2>Want to review your session analytics?</h2><p>See your biometric trends, time in each signal state, and check-ins.</p><div className="insight-actions"><button onClick={() => setMode('report')}>Show analytics</button><button onClick={() => setMode('dismissed')}>Not now</button></div></> : <>
      ...
      {error ? <div role="alert"><p>Could not load your session analytics. {error}</p><button type="button" onClick={()=>{setError('');setStats(null);setAttempt(n=>n+1);}}>Retry analytics</button></div> : !stats ? <p role="status">Loading analytics…</p> : <>
        <p>{Math.round(stats.durationSeconds / 60)} {Math.round(stats.durationSeconds / 60)===1?'minute':'minutes'} · {stats.sampleCount} samples · {stats.interventionCount} {stats.interventionCount===1?'check-in':'check-ins'}{session.source === 'demo' ? ' · Simulated data' : ''}</p>
        ...
        <CheckinEvidence evidence={session.interventions?.at(-1)?.evidence}/>
        ...
        <p className="insight-footnote">Steady/elevated states combine heart and breathing signals. A pulse-only rise can still prompt a check-in. Signal changes can have many causes. These are descriptive readings, not a measure or diagnosis of stress. Missing HRV is never estimated.</p>
