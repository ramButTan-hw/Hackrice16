...
export default function Chat({ visible, sessionId }) {
  ...
  async function send(event) {
    ...
    const context = [...messages.slice(-10).map(m => ({ ...m, text: m.text.slice(0, 2000) })), question];
    while (context.reduce((sum, m) => sum + m.text.length, 0) > 12000 && context.length > 1) context.splice(0, 2);
    try {
      const response = await fetch('/api/chat', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ messages: context, sessionId: share ? sessionId : undefined }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Unable to send message.');
      setMessages(previous => [...previous, question, { role: 'model', text: data.text }]); setDraft('');
  ...
  return <section className="chat" hidden={!visible} aria-label="Gemini chat">
    ...
    <div className="chat-log" ref={log} role="log" aria-live="polite" aria-relevant="additions">
      ...
      {messages.map((message, index) => <div className={'chat-message ' + message.role} key={index}><span>{message.role === 'user' ? 'You' : 'Gemini'}</span><p>{message.text}</p></div>)}
