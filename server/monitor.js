...
export function monitoringService({ sessions, now = Date.now, matlab = matlabWorker(), insight = analyzeInsight, speech = generateSpeech, configured = () => Boolean(process.env.GEMINI_API_KEY) }) {
  ...
  async function run(id, state) {
    ...
    if (!fresh(s)) {
      await sessions.update(id, s => { s.monitor.status = 'waiting_signal'; });
      return;
    }
    await sessions.update(id, s => { s.monitor.status = 'analyzing_matlab'; s.monitor.error = null; });
    const report = await matlab.analyze(s, now());
    if (!alive()) return;
    s = await sessions.get(id);
    if (s.status !== 'active' || !s.monitor.enabled) return;
    if (!fresh(s) || now() - report.timestamp > 15000) return;
    const ready = report.ready && now() - s.startedAt >= MONITOR.warmupMs;
    await sessions.update(id, s => { s.monitor.matlab = report; s.monitor.status = ready ? 'watching' : 'calibrating'; });
    if (!ready || s.samples.at(-1).quality < 0.7) return;
    ...
    if (gap < MONITOR.minimumMs || (gap < MONITOR.regularMs && !changed)) return;
    const snapshot = {
      task: s.goal, source: s.source, elapsedSeconds: Math.floor((now() - s.startedAt) / 1000),
      matlab: report,
      presage: s.samples.at(-1),
    ...
    await sessions.update(id, s => {
      ...
      s.monitor.calls++; s.monitor.lastCallAt = now(); s.monitor.lastSignature = signature; s.monitor.status = 'analyzing_gemini'; reserved = true;
    });
    ...
    try {
      for (const [id, state] of active) {
        try { await run(id, state); }
          ...
          await sessions.update(id, s => { s.monitor.status = 'error'; s.monitor.error = error.message; }).catch(() => {});
        }
      }
